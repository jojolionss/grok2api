...
// === Tavily Keys Management ===
adminRoutes.get("/api/tavily/keys", requireAdminAuth, async (c) => {
  try {
    const rows = await listTavilyKeys(c.env.DB);
    const infos = rows.map(tavilyKeyRowToInfo);
    return c.json({ success: true, data: infos, total: infos.length });
  } catch (e) {
...
adminRoutes.post("/api/tavily/keys/add", requireAdminAuth, async (c) => {
  try {
...
adminRoutes.post("/api/tavily/keys/delete", requireAdminAuth, async (c) => {
  try {
    const body = (await c.req.json()) as { keys?: string[] };
    const keys = Array.isArray(body.keys) ? body.keys : [];
    const deleted = await deleteTavilyKeys(c.env.DB, keys);
    return c.json({ success: true, message: `删除成功(${deleted})`, count: deleted });
  } catch (e) {
    return c.json(jsonError(`删除失败: ${e instanceof Error ? e.message : String(e)}`, "TAVILY_KEYS_DELETE_ERROR"), 500);
  }
});

adminRoutes.post("/api/tavily/keys/tags", requireAdminAuth, async (c) => {
  try {
    const body = (await c.req.json()) as { key?: string; tags?: string[] };
    const key = String(body.key ?? "");
    const tags = Array.isArray(body.tags) ? body.tags : [];
    await updateTavilyKeyTags(c.env.DB, key, tags);
    return c.json({ success: true, message: "标签更新成功", tags });
  } catch (e) {
    return c.json(jsonError(`更新失败: ${e instanceof Error ? e.message : String(e)}`, "TAVILY_UPDATE_TAGS_ERROR"), 500);
  }
});

adminRoutes.post("/api/tavily/keys/note", requireAdminAuth, async (c) => {
  try {
    const body = (await c.req.json()) as { key?: string; note?: string };
    const key = String(body.key ?? "");
    const note = String(body.note ?? "");
    await updateTavilyKeyNote(c.env.DB, key, note);
    return c.json({ success: true, message: "备注更新成功", note });
  } catch (e) {
    return c.json(jsonError(`更新失败: ${e instanceof Error ? e.message : String(e)}`, "TAVILY_UPDATE_NOTE_ERROR"), 500);
  }
});

adminRoutes.post("/api/tavily/keys/toggle", requireAdminAuth, async (c) => {
  try {
    const body = (await c.req.json()) as { key?: string; is_active?: boolean };
    const key = String(body.key ?? "");
    await updateTavilyKeyActive(c.env.DB, key, Boolean(body.is_active));
    return c.json({ success: true, message: body.is_active ? "已启用" : "已禁用" });
  } catch (e) {
    return c.json(jsonError(`更新失败: ${e instanceof Error ? e.message : String(e)}`, "TAVILY_TOGGLE_ERROR"), 500);
  }
});

adminRoutes.get("/api/tavily/tags/all", requireAdminAuth, async (c) => {
  try {
    const tags = await getAllTavilyTags(c.env.DB);
    return c.json({ success: true, data: tags });
  } catch (e) {
    return c.json(jsonError(`获取失败: ${e instanceof Error ? e.message : String(e)}`, "TAVILY_GET_TAGS_ERROR"), 500);
  }
});

adminRoutes.get("/api/tavily/stats", requireAdminAuth, async (c) => {
  try {
...
adminRoutes.post("/api/tavily/keys/test", requireAdminAuth, async (c) => {
  try {
    const body = (await c.req.json()) as { key?: string };
    const key = String(body.key ?? "");
    ...
    if (result.valid && typeof result.usage === "number") {
      ...
      return c.json({
        ...
        data: {
          ...
          usage: result.usage,
          limit: result.limit,
          remaining: (result.limit ?? 1000) - (result.usage ?? 0),
        },
      });
      ...
      return c.json({
        success: true,
        message: "Key有效但被限流",
        data: { valid: true, rate_limited: true },
      });
    } else {
      if (result.reason === "deactivated" || result.reason === "unauthorized") {
        await markTavilyKeyInvalid(c.env.DB, key, result.reason);
      }
      return c.json({
        success: false,
        message: `Key无效: ${result.reason}`,
        data: { valid: false, reason: result.reason },
      });
    }
  } catch (e) {
    return c.json(jsonError(`测试失败: ${e instanceof Error ? e.message : String(e)}`, "TAVILY_TEST_ERROR"), 500);
  }
});
...
adminRoutes.get("/api/tavily/keys/sync-progress", requireAdminAuth, async (c) => {
  try {
